...
public class DoingEncoder {
    ...
    private static final int      WIDTH             = 480;           // note
                                                                      // 480x640,
                                                                      // not
                                                                      // 640x480
    ...
    /**
     * Generates the presentation time for frame N, in nanoseconds. Fixed frame
     * rate.
     */
    ...
    protected void prepareEncoder(String mimeType, int width, int height, int bitRate, int framesPerSecond, File outputFile) throws IOException {
        ...
        try {
            ...
            // This is generally the first time we ever try to encode something
            // through a
            // Surface, so specialize the message a bit if we can guess at why
            // it's failing.
     ...
     * 
     * @param presentationTimeNsec
     *            The presentation time stamp, in nanoseconds.
     */
    protected void submitFrame(long presentationTimeNsec) {
        // The eglSwapBuffers call will block if the input is full, which would
        // be bad if
        // it stayed full until we dequeued an output buffer (which we can't do,
        // since we're
        // stuck here). So long as the caller fully drains the encoder before
        // supplying
        // additional input, the system guarantees that we can supply another
        // frame
        // without blocking.
