...
/**
 * Generate an MP4 file using OpenGL ES drawing commands. Demonstrates the use
 * of MediaMuxer and MediaCodec with Surface input.
 * <p>
 * This uses various features first available in Android "Jellybean" 4.3 (API
 * 18). There is no equivalent functionality in previous releases.
 * <p>
...
public class EncodeAndMuxTest {
    ...
    private static final int            IFRAME_INTERVAL = 5;                                                                           // 1
                                                                                                                                        // second
                                                                                                                                        // between
                                                                                                                                        // I-frames
    // public static final int TOTAL_FRAMES = 900; //30 seconds stand for 900
    // frames
    ...
    public void testEncodeVideoToMp4(SaveCallback callback, final ProgressDialog progressDialog) {
        ...
        // mBitRate = 7741440; // 1280 * 720 * 30 * 4(high motion, low
        // motion(5fps):1) * 0.07
        ...
        try {
            ...
            for(int i = 0; i < processNum && totalFrame < TOTAL_FRAMES && !Thread.currentThread().isInterrupted(); i++) {
                ...
                for(int j = 0; j < numFrame && !Thread.currentThread().isInterrupted(); j++) {
                    ...
                    if(elapseTime <= interval) {
                        ...
                        // Submit it to the encoder. The eglSwapBuffers call
                        // will block if the input
                        // is full, which would be bad if it stayed full until
                        // we dequeued an output
                        // buffer (which we can't do, since we're stuck here).
                        // So long as we fully drain
                        // the encoder before supplying additional input, the
                        // system guarantees that we
            ...
            if(Thread.currentThread().isInterrupted() || isException || mIsCancel) {
                ...
                if(mService != null) {
                    try {
                        result = mService.encodeAudio(OUTPUT_DIR, MusicManager.getResourceId(mContext, mScript.getMusicId()),
                                MusicManager.getFileName(mContext, mScript.getMusicId()), mVideoPath, mOutputPath, new ICallback.Stub() {
                                    @Override
        ...
        // To test the result, open the file with MediaExtractor, and get the
        // format. Pass
        // that into the MediaCodec decoder configuration, along with a
    ...
    private void setMovieOrder() {
        // mFileOrder = MicroMovieOrder.gettimeandorder(mEphotoApp, mFilesList,
        // mScript); // do not change order again
    ...
    private Uri exportToGallery(String filename) {
        ...
        final Uri uri = mContext.getContentResolver().insert(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, values);
    ...
    private static class CodecInputSurface {
        ...
        private void eglSetup() {
            ...
            int[] attribList = { EGL14.EGL_ALPHA_SIZE, 8, EGL14.EGL_RED_SIZE, 8, EGL14.EGL_GREEN_SIZE, 8, EGL14.EGL_BLUE_SIZE, 8,
