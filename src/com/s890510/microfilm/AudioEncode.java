...
public class AudioEncode {
    ...
    // public static void doEncode(Context context, String dstMediaPath,
    // MediaFormat videoFormat){
    public void doEncode(Context context, String dstMediaPath, MediaMuxer muxer) {
        int source = mAudioSourceId;
        try {
            // cloneMediaUsingMuxer(context, videoFormat, source, dstMediaPath,
            // 1, -1);
    ...
    public void setupAudioMuxer(Context context, MediaMuxer muxer, int time) {
        ...
        if(time == NO_TIME_LIMIT)
            ...
            mEncodeTime = time * 1000000; // sec to microsecond
    }

    /**
     * Using the MediaMuxer to clone a media file.
     */
    // private static void cloneMediaUsingMuxer(Context context, MediaFormat
    // videoFormat, int srcMedia, String dstMediaPath,
    private void cloneMediaUsingMuxer(Context context, MediaMuxer muxer, int srcMedia, String dstMediaPath, int expectedTrackCount, int degrees)
            throws IOException {
        ...
        while(!sawEOS) {
            ...
            if(bufferInfo.size < 0) {
                ...
                muxer.writeSampleData(mIndexMap.get(trackIndex), dstBuf, bufferInfo);
                ...
                frameCount++;
                if(VERBOSE) {
                    Log.d(TAG, "Frame (" + frameCount + ") " + "PresentationTimeUs:" + bufferInfo.presentationTimeUs + " Flags:" + bufferInfo.flags
                            + " TrackIndex:" + trackIndex + " Size(KB) " + bufferInfo.size / 1024);
                }
            }
        }
        // muxer.stop();
