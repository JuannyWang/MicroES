...
public class EffectLib {
    ...
    public static Effect String_Translate(int[] Duration, int Sleep, boolean[] Count, String mShader, ArrayList<String> str, boolean[] Trans,
            int[] mUtil, int ConvertType, int ConvertSize, float[] StartPosX, float[] EndPosX, float[] StartPosY, float[] EndPosY,
            float[] StartScale, float[] EndScale, float[] StartAlpha, float[] EndAlpha, float WRatio, float HRatio, int[] mMask, int[] mType,
            int size, int sType) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();
        ...
        for(int i = 0; i < Duration.length; i++) {
            Effect mEffect = null;
            if(mType[i] == 1) {
                mEffect = new EffectBySetting(Duration[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i], StartPosX[i], StartPosY[i],
                        EndPosX[i], EndPosY[i], mUtil[i], mMask[i]);
            }
    ...
    public static Effect Scale_Fade(ProcessGL processGL, int[] Duration, int Sleep, String mShader, boolean[] Trans, int[] mUtil, int ConvertType,
            int ConvertSize, float[] StartScale, float[] EndScale, float[] StartAlpha, float[] EndAlpha, float WRatio, float HRatio, int[] mMask,
            int[] mType) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();
    ...
    public static Effect Rotate_Translate(int[] Duration, int Sleep, String mShader, boolean[] Trans, int[] mUtil, int ConvertType, int ConvertSize,
            float[] StartPosX, float[] EndPosX, float[] StartPosY, float[] EndPosY, float[] StartScale, float[] EndScale, float[] StartAlpha,
            float[] EndAlpha, float[] StartRotate, float[] EndRotate, float WRatio, float HRatio, int[] mMask, int[] mRType, int[] mType) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();
        ...
        for(int i = 0; i < Duration.length; i++) {
            ...
            if(mType[i] == 1) {
                ...
                mEffect = new EffectRotate(Duration[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i], StartRotate[i], EndRotate[i],
                        mMask[i], mUtil[i], mRType[i]);
            } else if(mType[i] == 3) {
                mEffect = new EffectRotate(Duration[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i], StartRotate[i], EndRotate[i],
                        mMask[i], mUtil[i], mRType[i]);
                mEffect.setshowBackground(true);
    ...
    public static Effect Translate(ProcessGL processGL, int[] Duration, int Sleep, String mShader, boolean[] Trans, int[] mUtil, int ConvertType,
            int ConvertSize, boolean IsInCount, float[] StartPos, float[] EndPos, float[] StartScale, float[] EndScale, float[] StartAlpha,
            float[] EndAlpha, float WRatio, float HRatio, int[] mMask, int[] mType) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();

        for(int i = 0; i < Duration.length; i++) {
            Effect mEffect = null;
            if(mType[i] == 1) {
                mEffect = new EffectTranslateX(Duration[i], StartPos[i], EndPos[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i], mMask[i],
                        mUtil[i]);
            } else if(mType[i] == 2) {
                mEffect = new EffectScale(Duration[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i], mMask[i], mUtil[i]);
            } else if(mType[i] == 3) {
                ...
                mEffect = new EffectTranslateY(Duration[i], StartPos[i], EndPos[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i], 0,
                        mMask[i]);
                ...
                if(mType[i] >= 8 && mType[i] <= 10)
                    mEffect = new EffectTranslateY(Duration[i], StartPos[i], EndPos[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i], PosX,
                            mMask[i]);
                else if(mType[i] >= 11 && mType[i] <= 13)
                    mEffect = new EffectTranslateX(Duration[i], StartPos[i], EndPos[i], StartScale[i], EndScale[i], StartAlpha[i], EndAlpha[i],
                            mMask[i], mUtil[i]);
    ...
    public static Effect Cover(int[] Duration, int Sleep, String mShader, boolean[] Trans, int[] mUtil, int ConvertType, int ConvertSize, float x,
            float y, float[] StartScale, float[] EndScale, float[] StartAlpha, float[] EndAlpha, float WRatio, float HRatio, int[] mMask, int[] mType) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();
    ...
    public static Effect Bound(ProcessGL processGL, int[] Duration, int Sleep, String mShader, int ConvertType, int ConvertSize, float[] StartPosX,
            float[] EndPosX, float[] StartPosY, float[] EndPosY, float ShowPos, float EndPos, float[] StartScale, float[] EndScale,
            float[] StartAlpha, float[] EndAlpha, float WRatio, float HRatio, boolean[] Trans, int[] Bound, int[] mType) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();
    ...
    public static Effect Filter(int[] Duration, int Sleep, String mShader, float[] StartScale, float[] EndScale, float[] StartAlpha,
            float[] EndAlpha, float mRed, float mGreen, float mBlue, int[] mType) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();
    ...
    public static Effect Slogan(int Duration[], int Sleep, String mShader, float[] StartAlpha, float[] EndAlpha, int[] mMask, boolean[] Trans) {
        ArrayList<Effect> mElements = new ArrayList<Effect>();
