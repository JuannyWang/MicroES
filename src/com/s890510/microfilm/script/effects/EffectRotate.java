...
public class EffectRotate extends BasicEffect
{
    ...
    public EffectRotate(int duration, float startrate, float endrate, float startalpha, float endalpha,
            float startrotate, float endrotate, int mask, int util, int rtype) {
        mDuration = duration;
        mSleep = duration;
    ...
    public float[] getMVPMatrixByElapse(long elapse) {
        ...
        if(mUtil != 0) {
            scale = mStartScale + Util.Easing(mUtil, progress*mDuration, 0, 1, mDuration) * (mEndScale - mStartScale);
        } else {
        ...
        if(mRotate) {
            float rotate;
            if(mUtil != 0) {
                rotate = mStartRotate + Util.Easing(mUtil, progress*mDuration, 0, 1, mDuration) * (mEndRotate - mStartRotate);
            } else {
