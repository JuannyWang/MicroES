...
public class EffectSlideScaleFromLeft extends BasicEffect
{
    private final static int DURATION = 5000;
    private final float SCALE_RATE = 0.12f;
    private final float SLIDE_RATE = 0.1f;//SCALE_RATE must bigger than SLIDE_RATE
    private float[] mMVPMatrix = new float[16];
    private float scaleX = 0;
    public EffectSlideScaleFromLeft() {
        mDuration = DURATION;
    }

    public EffectSlideScaleFromLeft(String shader) {
        mDuration = DURATION;
        setShader(shader);
    }
    
    public EffectSlideScaleFromLeft(int duration) {
        mDuration = duration;
        mSleep = duration;
    }
    ...
    public float[] getMVPMatrixByElapse(long elapse) {
        ...
        scaleX = SLIDE_RATE*2*getProgressByElapse(elapse)-SLIDE_RATE;//-SLIDE_RATE ~ SLIDE_RATE
