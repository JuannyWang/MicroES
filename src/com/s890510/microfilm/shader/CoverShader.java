        ...
        if(mEffect.getTransition(mElapseTime)) {
            ...
            if(mCoverType == Shader.LEFT || mCoverType == Shader.HALF_LEFT ||
                ...
                if(mCoverType == Shader.LEFT || mCoverType == Shader.GFRAG_LEFT) {
                    GLES20.glUniform1f(mSizeHandle, Util.easeInOutCubic(elapse, 0, mProcessGL.ScreenRatio*2, duration) - mProcessGL.ScreenRatio);
                } else if(mCoverType == Shader.RIGHT) {
                    GLES20.glUniform1f(mSizeHandle, -(Util.easeInOutCubic(elapse, 0, mProcessGL.ScreenRatio*2, duration) - mProcessGL.ScreenRatio));
                } else if(mCoverType == Shader.HALF_LEFT) {
                    GLES20.glUniform1f(mSizeHandle, Util.easeInOutCubic(elapse, 0, mProcessGL.ScreenRatio, duration) - mProcessGL.ScreenRatio/2.0f);
                } else if(mCoverType == Shader.HALF_LEFT_Q) {
                    GLES20.glUniform1f(mSizeHandle, Util.easeOutCubic(elapse, 0, mElementInfo.x*2, duration) - mElementInfo.x);
                ...
                if(mCoverType == Shader.TOP) {
                    GLES20.glUniform1f(mSizeHandle, Util.easeInOutCubic(elapse, 0, 2, duration) - 1);
                } else if(mCoverType == Shader.BOTTOM) {
                    GLES20.glUniform1f(mSizeHandle, -(Util.easeInOutCubic(elapse, 0, 2, duration) - 1));
                } else if(mCoverType == Shader.HALF_TOP) {
                    GLES20.glUniform1f(mSizeHandle, Util.easeInOutCubic(elapse, 0, 1, duration) - 0.5f);
                } else if(mCoverType == Shader.HALF_BOTTOM) {
                    GLES20.glUniform1f(mSizeHandle, -(Util.easeInOutCubic(elapse, 0, 1, duration) - 0.5f));
                }
            } else if(mCoverType == Shader.CENTER_H) {
                GLES20.glUniform1f(mDirectHandle, 5.0f);
                GLES20.glUniform1f(mSizeHandle, 1-Util.easeInOutCubic(elapse, 0, 1, duration));
            } else if(mCoverType == Shader.PERCENT_L) {
                GLES20.glUniform1f(mDirectHandle, 0.0f);
                float[] mPos = mEffect.getRunPos(mElapseTime);
                GLES20.glUniform1f(mSizeHandle, (mPos[0] + ((mPos[1] - mPos[0])*Util.easeInOutCubic(elapse, 0, 1, duration)))*mProcessGL.ScreenRatio*2 - mPos[1]*mProcessGL.ScreenRatio);
            } else if(mCoverType == Shader.PERCENT_B) {
                GLES20.glUniform1f(mDirectHandle, 3.0f);
                float[] mPos = mEffect.getRunPos(mElapseTime);
                GLES20.glUniform1f(mSizeHandle, -((mPos[0] + ((mPos[1] - mPos[0])*Util.easeInOutCubic(elapse, 0, 1, duration)))*2 - mPos[1]));
