...
public class ElementInfo {
    ...
    public void CalcTriangleVertices(ProcessGL processGL) {
        ...
        if(mVisioRatio == mBitmapRatio) {
            ...
            if(mBitmapRatio > mVisioRatio) { // can't cut width, need to cut
                                             // height
        ...
        float[] mVerticesData = new float[] { -x, -y, 0.0f, x, -y, 0.0f, -x, y, 0.0f, x, y, 0.0f };

        float[] mTextCoordsData = new float[] { (centerX - textW / 2), (centerY - textH / 2), (centerX + textW / 2), (centerY - textH / 2),
                (centerX - textW / 2), (centerY + textH / 2), (centerX + textW / 2), (centerY + textH / 2) };
        ...
        mSTextureCoords = ByteBuffer.allocateDirect(mTextCoordsData.length * ProcessGL.FLOAT_SIZE_BYTES).order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        mSTextureCoords.put(mTextCoordsData).position(0);

        mCTextureCoords = ByteBuffer.allocateDirect(textures.length * ProcessGL.FLOAT_SIZE_BYTES).order(ByteOrder.nativeOrder()).asFloatBuffer();
